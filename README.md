...
插件除了能随机发送图片，还能在群聊内直接通过指令存储图片，同时为了避免重复加入同一张图片，该插件添加了图片感知哈希算法，相似的图片无法重复添加。
...
在插件的本地存储目录下会有 `randpic_commands.json` 文件，该文件存储了对应指令。群里添加新的指令会修改这个文件，你也可以自己修改然后重启bot。

例如 `randpic_commands.json` 格式如下：
...
|  `添加<指令>`  | 群管 |  否  | 群聊 |       让 bot 存储图片到对应文件夹下       |
